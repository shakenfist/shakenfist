...
* `pre-commit run --all-files` passes in kerbside (4a) — and in
  kerbside-patches when 4b runs.
* Post-merge tail (tracked, not blocking "implemented on
  branch"): patch175 contains only the override branch and
  applies cleanly via `_build/test-apply.sh`; a kerbside
  scenario job goes green on Gerrit from unmodified kolla
  master, build UUID recorded.
