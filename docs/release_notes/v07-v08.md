...
* **`sf-database` is a tier of N >= 1 instances.** All instances
  ...
  across the `MARIADB_GATEWAY_HOSTS` list, skipping unhealthy endpoints
  via the `grpc.health.v1.Health` protocol. No external L4 load
  balancer is required. Rolling upgrades work cleanly because each
  `sf-database` instance flips its health status to `NOT_SERVING`
  before stopping.
