...
* **`getsf` prompts for connection details.** The installer now asks for
  `GETSF_MARIADB_HOST`, `GETSF_MARIADB_PORT` (default `3306`),
  `GETSF_MARIADB_USER` (default `shakenfist`),
  `GETSF_MARIADB_PASSWORD`, and `GETSF_MARIADB_DATABASE` (default
  `shakenfist`). All five can be passed as environment variables for
  non-interactive deploys. The deployer no longer generates a password —
  operators choose it when they apply `tools/bootstrap-mariadb.sql`.
