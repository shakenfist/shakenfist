...
* **`sf-backup` and `sf-restore` have been retired.** These commands previously
  ...
  `mysqldump`) to export the Shaken Fist database, and separately back up
  the event log SQLite files under `/srv/shakenfist/events/` on the eventlog
  node. To restore, use `mariadb` (or `mysql`) to import the dump and copy
  the SQLite files back into place.
