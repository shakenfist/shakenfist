...
Expected demand affects placement directly: each successful
admission adds `vcpus × SCHEDULER_DEMAND_PER_VCPU` to the target
node's `expected_demand` counter in the same transaction, and the
admission guard refuses a node whose existing load is *already* above
its target:
