...
|-----------|--------|-------------|
| Instance create refused `507 sufficient_idle_cpu` under suite concurrency | #3772 (umbrella, **closed 2026-09-09**); #3498, #3602, #3670, #3728, #3749, #3767 (closed members) | **Behaviour to assert.** Phase 3 asserts what the server does today. *Correction (2026-09-10):* this row said "#3772 stays open: its own verdict is that a bare 507 is the wrong answer to a transient condition". The verdict is unchanged, but the issue is not open -- it was closed by hand on 2026-09-09 with no closing comment and no commit claiming it, hours after two fresh recurrences. The record of what a capacity refusal *should* do has moved into [PLAN-transient-capacity-refusals.md](PLAN-transient-capacity-refusals.md), which owns that change and which this plan deliberately does not. Phase 3 also no longer fills a whole cluster: see its D23 and D24, which fill one node and prove the refusal contract with a request no cluster could satisfy. |
| Instance create refused `507 sufficient_idle_disk`, a rate predicate rather than a ledger | #3772 (umbrella, open) | **Behaviour to assert; sizing does not address it.** `_has_idle_disk_bandwidth()` (`shakenfist/scheduler.py:495`) checks disk-busy rate against a metric unrelated to `cpu_schedulable` or memory. A 2026-08-21 comment on #3772 records an occurrence at exactly this stage on the single-node smoke topology, under a `pull_request` event, with the node hosting only two instances and every other capacity stage passing -- a cluster nowhere near full. Widening vCPUs does nothing for it. Phase 3's saturation test must cover this stage directly rather than assume the CPU-stage test also exercises it. *Correction (2026-09-10):* it cannot. The predicate reads a measured disk-busy rate republished on the resources daemon's 60 second cadence, against a 1200 ms/s threshold that `/admin/resources` does not publish at all, so no functional test can force it and none can observe why it did or did not fire. Phase 3's **D25** covers it with a unit test of the predicate instead, and records that the stage phase 0 singled out as the one sizing cannot fix is also the stage that phase least covers. |
