...
|-----------|--------|-------------|
| Instance create refused `507 sufficient_idle_cpu` under suite concurrency | #3772 (umbrella, closed 2026-09-09, **reopened 2026-09-10**); #3498, #3602, #3670, #3728, #3749, #3767 (closed members) | **Behaviour to assert.** Phase 3 asserts what the server does today. *Correction (2026-09-10):* this row said "#3772 stays open: its own verdict is that a bare 507 is the wrong answer to a transient condition". The verdict is unchanged, and the issue is open after all: it was closed by hand on 2026-09-09 with no closing comment and no commit claiming it, hours after two fresh recurrences, and **reopened on 2026-09-10 once that closure was determined to be an error** -- three further recurrences postdate it, one of them with the warm-up fix demonstrably in effect. The record of what a capacity refusal *should* do is shared with [PLAN-transient-capacity-refusals.md](PLAN-transient-capacity-refusals.md), which owns making a refusal transient and which this plan deliberately does not. Phase 3 also no longer fills a whole cluster: see its D23 and D24, which fill one node and prove the refusal contract with a request no cluster could satisfy. *Correction (2026-09-10, step 3f):* the six numbers in the Issues column are two kinds of thing, and reading them as six fixes badly overstates what is resolved now that the umbrella is open again. Only **#3498** closed `COMPLETED`, on its own, on 2026-08-13. The other five -- #3602, #3670, #3728, #3749 and #3767 -- all closed `NOT_PLANNED` within seconds of each other on 2026-08-15, a bulk fold into the umbrella; a `NOT_PLANNED` closure asserts nothing about a fix, so with #3772 reopened those five are open questions wearing a closed badge. **The census supports the disposition emphatically.** Across phase 2's baseline the `sufficient_idle_cpu` stage aborted a schedule 79 times in 49 of 204 job-runs (24%) and dropped 1,865 of 23,170 candidate evaluations (8.0%), concentrated on `slim-tier` (35 of 50 job-runs, 70%, 1,189 drops) rather than `slim-primary` (14 of 154, 9.1%, 676 drops); 44 of the 49 job-runs carrying an abort failed, and 44 of the window's 67 job failures carried one. The confirmation window repeats it at a twentieth of the size: 11 aborts in 7 of 32 job-runs, and all 7 of those job-runs failed. What the census cannot say is *which* ledger refused -- the predicate emits one reason string, `would exceed hard max CPUs`, whether the capacity row's `limit_cpus` or the live derivation bound (`scheduler.py:372-383`), and the summary record keeps the string rather than the payload's `capacity_row_present`. **Discharged by tests that now exist**, which is weaker than discharged by a run: `TestSaturationRefusals.test_impossible_cpu_request_refused_at_sufficient_idle_cpu` (3b) for the contract, `TestNodeFillRefusal.test_full_node_refuses_a_targeted_create` (3c) for the ledger, and `HasSufficientCPUPredicateTestCase` (`shakenfist/tests/test_scheduler.py:1930`, 3d) for the boundary. Neither functional test has ever executed against a cluster; a merge run using them is what turns "the coverage exists" into "the coverage holds". |
| Instance create refused `507 sufficient_idle_disk`, a rate predicate rather than a ledger | #3772 (umbrella; closed 2026-09-09, reopened 2026-09-10) | **Behaviour to assert; sizing does not address it.** `_has_idle_disk_bandwidth()` (`shakenfist/scheduler.py:495`) checks disk-busy rate against a metric unrelated to `cpu_schedulable` or memory. A 2026-08-21 comment on #3772 records an occurrence at exactly this stage on the single-node smoke topology, under a `pull_request` event, with the node hosting only two instances and every other capacity stage passing -- a cluster nowhere near full. Widening vCPUs does nothing for it. Phase 3's saturation test must cover this stage directly rather than assume the CPU-stage test also exercises it. *Correction (2026-09-10):* it cannot. The predicate reads a measured disk-busy rate republished on the resources daemon's 60 second cadence, against a 1200 ms/s threshold that `/admin/resources` does not publish at all, so no functional test can force it and none can observe why it did or did not fire. Phase 3's **D25** covers it with a unit test of the predicate instead, and records that the stage phase 0 singled out as the one sizing cannot fix is also the stage that phase least covers. *Correction (2026-09-10, step 3f):* **the census cannot speak to this row at all, and that is the finding.** The `sufficient_idle_disk` stage was reached 22,977 times across the baseline and 3,827 times across the confirmation window and dropped exactly zero candidates in both; `census.disk_bandwidth_drops` is 0 on all 236 records that carry a census at all (204 of the baseline's 217, and all 32 of the addendum's). There is therefore no measured frequency to weigh phase 0's disposition against, and none may be inferred from the absence: the census watches only `merge_group` runs on the two instrumented cluster topologies, while the occurrence phase 0 argues from was a `pull_request` event on the single-node smoke topology, which the instrument never watched. The disposition stands on its original reasoning rather than on data. Discharged, in the weaker form D25 chose, by `HasIdleDiskBandwidthPredicateTestCase` (`shakenfist/tests/test_scheduler.py:2159`, 3d), which pins 1200 ms/s as idle and 1201 as saturated and pins the reason dict the audit event publishes. There is deliberately no functional coverage, so no merge run can settle this row; only a recurrence can, and the census would still show the drop rather than the rate behind it. |
| Claim growth refused because the concurrent suite holds the cluster | #3907 (closed) | **Test bug, already fixed**, by routing success-asserted claim requests through a transient-tolerant retry. Phase 3 keeps a test that exercises the refusal path directly, so the retry cannot mask a genuine regression in claim admission. *Correction (2026-09-10, step 3f):* #3907 is closed `COMPLETED` (2026-08-27), and **this row was already discharged before phase 3 began**, by `cluster_ci_tests/test_namespace_claims.py`'s `test_claim_lifecycle_and_refusals` -- not by anything this phase wrote. Phase 3's relationship to it runs the other way: 3b copied that file's `IMPOSSIBLE_CPUS` shape for its own three refusals. The census answers this row only by half. The baseline is silent (guard `state` is `not_collected` on all 204 records, a fact about the LogQL filter and not about the cluster), and the confirmation window sees claims but no claim *refusal*: 24 claim events over 32 job-runs, one per namespace across 24 distinct `ci-claimaccount-*` namespaces, every one on all three dimensions, with zero `claim_malformed`. Those are placements admitted over an advisory claim, never refusals, and the filter matches no event for a claim create or grow that was refused. The data is consistent with "already fixed" -- the ambient refusal does not appear -- while being incapable of counting the signature either way. |
| Soft affinity loses to resource filters; instances that should share a node do not | #3565 (closed 2026-08-31) | **Test bug**, not here -- originally classified *defect to fix*; see the 2026-09-01 correction at the end of this cell. This is the most frequent `slim-primary` failure in the sampled window. A 2026-08-26 comment on the issue traces the most recent occurrence to `sufficient_idle_memory`, not `sufficient_idle_cpu` -- the candidate shapes consolidate onto fewer, larger hypervisors, which raises instances per node, so growing the cloud may relax the CPU filter while tightening the memory one instead of making the test uniformly greener. Fixing the scheduler is still not this plan's job. Phase 3 gives it a deterministic reproduction (fill the affinity target, then place) so it stops depending on ambient load. *Correction (2026-09-01):* the original disposition, "defect to fix", did not survive contact with a full trace, and the lede above has been changed to the classification that replaced it. scheduler-reservations phase 6 closed #3565 on a test change: the candidate set had collapsed to a single node before affinity was scored, so there was no tiebreak to lose and no scheduler defect behind it. The deterministic reproduction phase 3 owes it is still worth having -- it is now coverage of a documented guarantee rather than a hunt for a bug -- but this row's classification should be read as *test bug*. *Correction (2026-09-10, step 3f):* state, census and links only; the 2026-09-01 classification above is not revisited. #3565 is closed `COMPLETED` (2026-08-31). Two census readings bear on this row's evidence rather than on its verdict. First, "the most frequent `slim-primary` failure in the sampled window" must not be read into the census's most frequent abort: `affinity_constraints` aborts in 114 of 204 job-runs, more often than any other stage, but 71 of those 114 job-runs **passed**, and its only reason string -- `no co-located instance carries a required tag`, 476 drops -- is the refusal `cluster_ci_tests/test_scheduler.py`'s `test_unsatisfiable_require_with_tag_is_refused` exists to produce. Frequency of abort is not frequency of failure. Second, the 2026-08-26 comment's memory reading draws no support here: `sufficient_idle_memory` dropped no candidate in 23,091 baseline evaluations and none in 3,851 more in the confirmation window, and `sufficient_free_disk` likewise dropped none -- though the census window opens on 2026-08-30, the day before the issue closed, so it is almost entirely post-fix and could not have measured the original signature whatever it showed. **The deterministic reproduction this row asks for was not written.** Phase 3 delivered a node-fill refusal test rather than an affinity reproduction, so coverage stays `test_affinity` and `test_binary_affinity_prefers_the_tagged_node` in `cluster_ci_tests/test_scheduler.py`; `TestNodeFillRefusal` (3c) is the fill primitive such a reproduction would now be built from. This is the one row in the table that named a test and still does not have one. |
| Targeted create silently lands on a different node | #3496 (closed 2026-08-29) | **Defect to fix**, not here. Related but distinct: `force_placement` tests cannot fall back to a less loaded node, so they fail first under scarcity. Record the interaction; phase 3 asserts the documented behaviour of a targeted create against a full node. Deliberately **not** given a heads-up comment, unlike #3565 and #3772: this issue's recorded occurrence is a wrong-node placement with no refusal and no stage trace at all, and the one capacity mode it names it hands off to #3772, so a note predicting that it will reproduce less often would assert a connection its own evidence does not carry. *Correction (2026-09-10, step 3f):* #3496 is closed `COMPLETED` (2026-08-29). **The census cannot speak to this row**, by construction: a `force_placement` create runs the same stage pass as any other, so its drops are pooled with everything else, and nothing in the summary record names a request's target node or records a wrong-node placement. The signature phase 0 described -- a placement with no refusal and no stage trace -- has no counter in the dataset and never could have had one. Discharged by a test that now exists: `TestNodeFillRefusal.test_full_node_refuses_a_targeted_create` (3c) fills one hypervisor to its published `cpu_limit` and asserts what a targeted create is then answered with. D28's 2026-09-10 correction is this row's, and matters here more than anywhere: there are three answers rather than two, because there are two distinct 507s, and the test asserts the stage refusal, skips on the capacity guard's 507 (an invalid premise, not a wrong answer), and asserts the asynchronous `Requested node lacks resources` abort should it ever arrive. **Which path a real cluster answers with has never been observed** -- 3c's central instruction, and the thing in this table most genuinely pending a merge run. That run's recorded detail is what settles whether an operator naming a full node meets a 507 at request time or an errored instance seconds later, which is the difference [PLAN-transient-capacity-refusals.md](PLAN-transient-capacity-refusals.md) would want to know. |
| Demand guard refuses every placement below four schedulable threads | #3813 (closed) | **Fixed, and the reason for D1.** No further coverage needed beyond keeping a fleet where `cpu_schedulable` is small, which D1 does. *Correction (2026-09-10, step 3f):* #3813 is closed `COMPLETED` (2026-08-22) and its fix is not in question, but **"no further coverage needed" is the disposition in this table the data most questions.** The baseline could not see the guard at all (`state` `not_collected` on all 204 records). The confirmation window can, and it counts **3,480 guard denials over 32 job-runs** -- a median of 121 per `slim-primary` job-run and 134.5 per `slim-tier` one, 108.75 on average -- of which 3,477 name `demand` as their sole exceeded dimension against 3 naming `cpus`, split 1,789 `demand_measured_alone` and 1,688 `demand_estimate_tipped`, with zero `unenforced`, `malformed`, `empty_dimensions`, `nothing_exceeded` and `demand_unsplit`. Nearly every refusal this cluster issues is still a demand refusal. What keeps that from being a failure is the waive-and-re-walk both placement walks perform when every refusal was demand-only, not anything #3813 changed, and the census cannot say how often the waive fails to save the create: the two outcome events are unmatched by the filter, which is exactly what D29's step 3e widens and which has not merged. No test in this phase covers it either -- the demand clause is `_demand_guard_clause()` in `mariadb.py`, inside the atomic admission, while 3d's `HasSufficientCPUPredicateTestCase` pins the `scheduler.py` pre-filter, a different arithmetic in a different place. A merge run carrying the widened filter is what would settle whether that rate of denial ever reaches a user. |
| Runners lose communication when concurrent merge groups saturate the under-cloud | #3696 (closed 2026-09-05), #3718 (open) | **Out of scope** per D8; the family has two observed shapes -- a runner dying mid-test, and a nested node never booting far enough to answer SSH. Named so that a later reduction in footprint can be checked against it. In the same window as the two dead runners, three other attempts by the same PR failed with `507 sufficient_idle_cpu` (citing #3498 and #3670), which suggests the under-cloud shortage and the inner-cloud 507s are the same shortage observed at two levels -- a reading the issue itself calls "well supported by the surrounding evidence" rather than proven. *Correction (2026-09-10, step 3f):* #3696 is closed `COMPLETED` (2026-09-05); **#3718 is still open**, which together with the reopened #3772 makes two open issues in this table rather than one. **The census cannot speak to this row either**, and the reason is worth recording: the instrument runs inside the nested cluster and reports only from runs that banked a bundle, so a runner dying mid-test destroys the very evidence that would count it. What the window's shape does say is that no run in it is attributable to the signature -- 11 of 66 enumerated merge runs banked no bundle, and all 11 are accounted for by three cancellations and eight `Check paths` skips. That is an absence of evidence, not evidence of absence, so the "same shortage at two levels" reading stays exactly as well supported as it was and the disposition is unchanged: out of scope per D8, with phase 4's footprint reduction still the thing to check it against. One correction to the row's evidence: of the two inner-cloud 507 issues it cites, #3670 is one of the five `NOT_PLANNED` folds identified in this table's first row, so neither cited issue records a fix. |
...
*Re-checked (2026-09-10, phase 3 step 3f).* Every row above now carries
a correction refreshing its issue state, weighing its disposition
against phase 2's census in `docs/plans/data/ci-cloud-sizing-baseline/`,
and naming the test that discharges it. Two cautions about how those
discharges should be read. **A row is discharged by a test that now
exists, which is a weaker claim than a row discharged by a run**: none
of the four functional tests named above -- 3b's three and 3c's one --
has ever executed against a cluster, they are committed and unmerged,
and this phase's own step ordering wanted 3f to follow a merge run that
has not happened. And **the census answers three of the seven rows, half
of a fourth, and none of the remaining three.** It counts the CPU
umbrella, the affinity row's evidence and the demand guard; it sees
claim exceedances but no claim refusal; and it cannot see disk
bandwidth (a stage that dropped nothing in 26,804 evaluations), a
targeted create's target node, or a runner that died before its bundle
was banked. A row the data cannot reach is not a row the data supports.
