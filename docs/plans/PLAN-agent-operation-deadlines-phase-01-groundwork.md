...
# 3. No command-name branching, and no dispatch methods, survive on
#    the job -- every dispatch body now lives on a handler class.
test 0 -eq "$(grep -c "cmd\['command'\] ==" shakenfist/daemons/sidechannel/main.py)"
test 0 -eq "$(grep -c 'def _dispatch_' shakenfist/daemons/sidechannel/main.py)"
