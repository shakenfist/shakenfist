...
A twelfth round found the fix from the eleventh had recreated a
subtler defect: making `security` a spec-valid array without gating
it on `requires_auth` published `/auth/federated` -- deliberately
unauthenticated, the identity token is the credential -- as
requiring a bearer token, and did so in a form no linter would ever
flag again. The requirement now travels with the Authorization
parameter inside the `if requires_auth:` block, and a test couples
the two in both directions. The round's other fix guarded the
harness's `mktemp -d` (an unchecked failure would have resolved
`cp` into the filesystem root).

That round was also where the loop was deliberately stopped. The
core ~120 corrections had been untouched since the seventh round,
each round's `fix` items were being generated by machinery the
previous round added, and the reviewer is a generator with no
notion of "done". The two remaining considers were filed as issues
rather than fixed (#3642: variadic handlers pass the
accepted-parameters check vacuously; #3643: the test module's own
`open()` calls are locale-dependent), and the suggestion to move
`declarations.py` into `tools/` was declined -- the placement is
defended in the module docstring, and the checkout-versus-wheel
invariant it creates is asserted by a test rather than assumed.
