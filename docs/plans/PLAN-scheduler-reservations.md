...
- **Phase 4c**'s steps 1 to 4 merged to private-ci `master` as
  ...
  **This answers phase 5's gate: do not flip
  ...
  discharged on 2026-09-08 against the window's own data. It
  went the right way: the arithmetic is correct on three real
  `(repo, job_name)` pairs. One definition-of-done item was
  retired rather than met: it asked for consistency against the
  conductor's own documentation of claims, which does not exist.
  The phase is not blocked on it and the writing of that
  documentation passes to the successor plan.
