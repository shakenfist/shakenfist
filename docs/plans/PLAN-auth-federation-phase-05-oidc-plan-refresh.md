...
- [ ] Every `docs/glossary.md` anchor cited by the rewrite resolves,
      verified by hand — `tools/check-doc-anchors.py` excludes
      `docs/plans/`, so pre-commit will not catch a broken one.
