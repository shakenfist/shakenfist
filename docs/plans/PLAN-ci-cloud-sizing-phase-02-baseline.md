...
### D17 -- Every cluster job, not only the two named topologies

**Decision:** the harvest covers all five functional cluster
bundles per merge run (`Debian 12 cluster`, `Ubuntu 24.04 cluster`,
`Guests`, `Debian 12 tier`, `Ansible modules`) plus `Node
lifecycle`, and reports per job as well as per topology.

**Reasoning:** the master plan's failure table is per job and the
spread across it is large -- 92% for node lifecycle down to 19% for
the tier -- while three of those jobs run the *same* topology. If
`slim-primary`'s jobs differ from each other in peak demand, the
difference is the suite, not the shape, and phase 4 must not
respond to it by resizing the cloud. Pooling by topology would hide
that. It also costs nothing: the bundles are downloaded per run
anyway.
