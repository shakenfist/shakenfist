...
4. **The proto-freshness check applies, unlike in the precedent.**
   ...
   `database_pb2.pyi`, and adds a counter in
   `shakenfist/daemons/database/main.py` (+10) alongside 37 changed
   lines in `shakenfist/mariadb.py`. So `tox -e genprotos` followed by
   `git diff --exit-code shakenfist/protos` is in wave 1 here, and the
   three-layer direct/gRPC/public review in 2a has something real to
   look at.
