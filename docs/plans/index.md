...
| [Remove the primary node](PLAN-remove-primary.md) | Phase 1: Remove monitoring | Complete (pending CI) | Realised by [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) phase 5 (Grafana and the primary-node Prometheus server already removed as warmup) |
...
| [Remove the primary node](PLAN-remove-primary.md) | [Phase 6: Galaxy collection](PLAN-remove-primary-phase-06-galaxy-role.md) | In progress (steps 1-5 authored; 5b awaiting green CI, step 6 remains) | Repackage deployer as the `shakenfist.shakenfist` collection (parameterised `node` role + component roles); native ansible modules under `plugins/modules/`; delete the getsf/topology installer chain. Step 5a (the `release.yml` collection build/publish jobs) is done and locally validated. Step 5b is authored: the reusable `smoke-cluster.yml` + `tools/` live on `shakenfist/actions` `main`, and `functional-tests.yml` calls it beside the getsf job — pending an operator push and a green CI run before the getsf job and legacy chain (step 6) are removed |
...
| [Remove the primary node](PLAN-remove-primary.md) | Phase 8: Roll out shared smoke-cluster CI | Not started | Roll the reusable `smoke-cluster.yml` (authored in phase 6 step 5) out to `client-python`/`library-python`/`kerbside`, replacing each repo's copy-pasted cluster-build CI with a few-line `uses:` call |
...
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 3: CI Loki](PLAN-remove-syslog-forwarding-phase-03-ci-loki.md) | Complete (pending CI) | `tools/ci-install-loki.sh` stands up Loki on the primary, `LOKI_BASE_URL` plumbed through getsf/deploy into `/etc/sf/config`, and `test_loki.py` asserts SF logs reach Loki end-to-end |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 4: Rework CI tooling](PLAN-remove-syslog-forwarding-phase-04-ci-tooling.md) | Complete (pending operator PR + CI) | New versioned `shakenfist/actions` log-checks querying Loki (structured `\| json` LogQL) + clingwrap bundle dumping the logship spool, per-node journald, and Loki (so a shipper failure stays debuggable); system-origin checks deferred to a phase-5 per-node check |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 5: Remove rsyslog forwarding](PLAN-remove-syslog-forwarding-phase-05-remove-rsyslog.md) | Complete (pending CI) | Atomic cut-over: delete rsyslog configs/`syslog_target`/install, route gunicorn logs + drop `--log-syslog`, add tenant/auth plumbing, switch both CI workflows to the phase-4 Loki tooling (realises remove-primary phase 1) |
