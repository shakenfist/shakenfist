...
| [Remove the primary node](PLAN-remove-primary.md) | Phase 6: Galaxy collection | Not started | Repackage deployer as the `shakenfist.shakenfist` collection (parameterised `node` role + component roles); native ansible modules under `plugins/modules/`; delete the getsf/topology installer chain |
