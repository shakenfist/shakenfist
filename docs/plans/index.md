...
| [Remove the primary node](PLAN-remove-primary.md) | [Phase 6: Galaxy collection](PLAN-remove-primary-phase-06-galaxy-role.md) | In progress (steps 1-4 of 6) | Repackage deployer as the `shakenfist.shakenfist` collection (parameterised `node` role + component roles); native ansible modules under `plugins/modules/`; delete the getsf/topology installer chain |
