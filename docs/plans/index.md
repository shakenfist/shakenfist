...
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 5: Remove rsyslog forwarding](PLAN-remove-syslog-forwarding-phase-05-remove-rsyslog.md) | Not started | Atomic cut-over: delete rsyslog configs/`syslog_target`/install, route gunicorn logs + drop `--log-syslog`, add tenant/auth plumbing, switch both CI workflows to the phase-4 Loki tooling (realises remove-primary phase 1) |
