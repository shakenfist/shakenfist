...
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 0: Decisions and design](PLAN-remove-syslog-forwarding-phase-00-decisions.md) | Not started | Config surface, Loki label/field-name contract, spool factoring, auth, CI Loki topology (delivers the Loki-shipper story remove-primary phase 1 awaits) |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | Phase 1: Default structured JSON logging | Not started | Flip `shakenfist_utilities` logging to JSON by default, document the field-name contract, add logs-module tests, cut a release and bump the pin |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | Phase 2: Loki shipper in shakenfist | Not started | `logship_spool.py` / `logship_drainer.py` + a `logging.Handler` modelled on the eventlog spool/drainer; HTTP push, config, lifecycle wiring, metrics |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | Phase 3: CI Loki | Not started | Stand up Loki for the functional cluster (as MariaDB is) and a functional test asserting SF logs reach Loki end-to-end |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | Phase 4: Rework CI tooling | Not started | New versioned `shakenfist/actions` log-checks querying Loki + clingwrap bundle dumping Loki and per-node local logs (so a shipper failure stays debuggable) |
