...
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 1: Default structured JSON logging](PLAN-remove-syslog-forwarding-phase-01-json-logging.md) | Not started | Flip `shakenfist_utilities` logging to JSON-only (v0.9.0), document the field-name contract, add logs-module tests, supersede the library's draft, cut a release and bump the pin |
