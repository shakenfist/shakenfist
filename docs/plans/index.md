...
| [Remove the primary node](PLAN-remove-primary.md) | [Phase 6: Galaxy collection](PLAN-remove-primary-phase-06-galaxy-role.md) | In progress (steps 1-4 + 5a of 6; 5b/6 remain) | Repackage deployer as the `shakenfist.shakenfist` collection (parameterised `node` role + component roles); native ansible modules under `plugins/modules/`; delete the getsf/topology installer chain. Step 5a (the `release.yml` collection build/publish jobs) is done and locally validated; step 5b (the reusable `smoke-cluster.yml` + functional-tests CI cutover, cross-repo) and step 6 (delete the legacy chain) remain |
