...
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 0: Retire etcd machinery | Not started | Supersedes `PLAN-remove-etcd`; deletes `etcd.py`, `etcd3gw`, etcd protos, `DATA_MIGRATIONS` framework, drain tests, `is_etcd_master`, dead `sf-ctl` helpers and `show/set-etcd-config` aliases, stale `migrate-*` comments, `.claude/skills/migrate-etcd-to-mariadb.md` |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 1: Statelessness and scope shift | Not started | Schema-versions lock; stop sf-database calling `ensure_schema()` at startup; daemon startup-version check; MariaDB compat check (version/engine/charset) on `ensure-mariadb-schema` and sf-database; lift scope out of remove-primary |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 2: Config untangle | Not started | Untangle `MARIADB_HOST` from "I am the database node"; rename `DATABASE_NODE_IP` → `MARIADB_GATEWAY_HOSTS` (plural list) |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 3: gRPC tier | Not started | Multi-endpoint client-side gRPC LB; minimal `grpc.health.v1.Health` on sf-database; channel factory |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 4: Deploy BYO | Not started | `getsf` prompts; delete `roles/mariadb/`; ship `tools/bootstrap-mariadb.sql`; remove generated password |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 5: CI workflow step | Not started | CI installs MariaDB and applies the SQL snippet before getsf runs |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 6: CI tier coverage | Not started | N=2 sf-database CI deployment shape with functional LB-path test |
| [BYO MariaDB and sf-database tier](PLAN-byo-mariadb.md) | Phase 7: Documentation | Not started | `docs/operator_guide/database.md` rewrite; ARCHITECTURE / README / AGENTS; breaking-change release notes |
