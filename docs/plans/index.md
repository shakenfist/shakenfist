...
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 1: Default structured JSON logging](PLAN-remove-syslog-forwarding-phase-01-json-logging.md) | In progress | Library JSON-only work done on branch `json-only-logging` (clean record, field contract, tests, draft superseded); awaiting operator `v0.9.0` release then the pin bump |
| [Remove syslog forwarding (ship logs to Loki)](PLAN-remove-syslog-forwarding.md) | [Phase 2: Loki shipper in shakenfist](PLAN-remove-syslog-forwarding-phase-02-loki-shipper.md) | Not started | `logship_spool.py` / `logship_drainer.py` + a `logging.Handler` modelled on the eventlog spool/drainer; HTTP push, config, lifecycle wiring, metrics |
