...
6. **A retry does not delete the blob the abandoned attempt
   ...
   (`main.py:839`) handles the *incomplete* file, which is the case
   that would otherwise leave bytes on disk with no object at all.

   **Corrected during review** (item 4): phase 4 wired that teardown
   into `expire_if_out_of_budget()` only, not into the executor's
   `finally`, so this decision as originally written was true of the
   budget path and false of the exit path this phase adds a retry to.
   Since each retry mints a fresh blob uuid in
   `GetFileCommand.dispatch()`, a connection dropped mid-transfer
   would have left one orphaned `.partial` per attempt. The `finally`
   now calls it unconditionally.
...
their source above rather than only here.

| # | Item | Outcome |
|---|---|---|
| 1 | The executor and the instance's monitor shared an abort file | Fixed. `SideChannelJob.__init__()` takes an `abort_name`, so the derivation happens once instead of being overwritten by a subclass afterwards. The reaper's abort no longer also stops the monitor, and building an executor no longer clears the monitor's abort file during shutdown. |
| 2 | Attempts were burned by dispatches which never reached the agent | Fixed, and decision-bearing: see the correction in step 5c's brief. |
| 3 | The config description promised a bound the option did not provide | Fixed. It now says the cap bounds retries of an operation which reached the agent, and says explicitly that it is not a bound on dispatch. The unbounded case it used to claim is the *Future work* entry below. |
| 4 | The executor's `finally` left a half finished get-file behind | Fixed; see the correction in decision 6. |
| 5 | The executor was registered after its thread started | Fixed. A replaced dispatcher generation could otherwise reap an operation which was actively running. |
| 6 | The reaper's abort test could not catch a wrong abort path | Fixed. The reaper asserts against the path a real executor derives, and `SideChannelAbortPathTestCase` ties that constant to the real derivation using real objects. |
| 7 | The cap accepted 0 and negative values | Fixed with `ge=1`, so a value which silently disables retry is refused at startup. |
| 8 | A test patched `time.sleep` on the shared module object | Fixed. The wait tests no longer patch it at all: an abort path which is already set is checked before the first sleep. |
| 9 | `reaper_attempts` was never pruned | Fixed in `reap_instance_monitors()`, alongside the monitor entry whose absence makes the rate limiter meaningless. |
