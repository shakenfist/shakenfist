...
5. No list of stage names in either new tool can suppress a stage
   from the census tally. The four capacity stages *are* named, for
   annotation and for classifying the D3 warning -- D10 requires
   that -- so a grep for them is not the check. The check is that a
   stage string the tool has never seen is still counted and still
   printed, pinned by a test, and that adding a
   `if stage not in <the map>: continue` filter to the tally makes
   that test fail. *(This item originally demanded no hardcoded
   stage names at all, which contradicted D10 and would have failed
   a correct implementation; step 1c caught it.)*
