...
* **`roster_key` earned its place immediately.** The second run's
  first wait records `node` as the name `sf1` and `roster_key` as that
  node's uuid -- a name pin resolved into the uuid-keyed roster. (The
  uuid itself is not quoted here: a bare uuid after a `key:` reads as a
  high-entropy secret to the credential scan, and the reading costs
  nothing to avoid.) That is exactly the case the
