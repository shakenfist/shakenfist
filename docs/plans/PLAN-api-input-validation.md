...
| 7: Structured parameter schemas | Not started | `dict` compiles to `fields.Dict()` and `arrayofdict` to `fields.List(fields.Dict())`, neither carrying a value schema, so every key inside a diskspec, networkspec or videospec is unvalidated in every mode — confirmed by sending requests: eleven nested values are a recorded 500 today and nine more are accepted in silence, including an unknown key, a negative disk size and a `float` of the string `"yes"`. The population is smaller than the row used to suggest: six declarations across four operations, of which `metadata` and `bound_claims` stay out (one is free-form by design, the other already guarded by hand better than a schema could be). Teaching the vocabulary to carry an element schema is what #528 needs — the scope #936 described before it was closed as a duplicate of #528 on 2026-09-12, although #528's own text describes `use_kwargs` coverage that phase 4 already overtook, which this phase corrects — and it is also items 2 and 3 of #4167 and the nested half of #3612. Split out of phase 6 by that phase's survey: the other issues in that row are guards and scalar validators. See [phase 7](PLAN-api-input-validation-phase-07-structured.md) | — |
