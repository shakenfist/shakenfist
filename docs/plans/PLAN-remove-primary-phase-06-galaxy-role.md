...
| 5 | high | opus | worktree | **Stand up the new CI path beside the old; add the release job.** (a) In `.github/workflows/functional-tests.yml` and `scheduled-tests.yml`, add a new matrix entry / job that deploys via the collection: `ansible-galaxy collection install ./shakenfist/deploy/collection` (or set `ANSIBLE_COLLECTIONS_PATH`), then `ansible-playbook -i examples/<topology>/inventory.yaml examples/<topology>/site.yml`, then run the existing `shakenfist_ci` test suites against it. Keep the existing `getsf` job intact in parallel so CI never goes dark; the new job proves the new path green before step 6 removes the old. (b) Add a `build-collection` job to `.github/workflows/release.yml` alongside the PyPI job: inject the SF version into `galaxy.yml` (`sed`/yq from the setuptools_scm version), `ansible-galaxy collection build shakenfist/deploy/collection`, `ansible-galaxy collection publish` with `${{ secrets.ANSIBLE_GALAXY_TOKEN }}`. (c) **Flag for the operator (cannot be done by a sub-agent):** the `shakenfist/actions` repo must gain the matching new-path plumbing — replace the `getsf-wrapper` step with the `ansible-galaxy collection install` + `ansible-playbook examples/...` invocation, and replace the `ci-topology-*.yml` topology JSON with a plain ansible inventory matching the example shape; and the `shakenfist` galaxy namespace + `ANSIBLE_GALAXY_TOKEN` secret must exist before the release job runs. The brief output must list these actions-repo + secret prerequisites explicitly. Keep workflow scripts >5 lines in `tools/` per repo convention. `pre-commit run --all-files` (actionlint). Worktree. |
