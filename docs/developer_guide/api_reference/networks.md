...
for a given network have been exposed. This lists the various pieces of queued
...
???+ tip "REST API calls"
    ...
    * [GET ​/networks/{network_ref}​/clusteroperations](https://openapi.shakenfist.com/#/networks/get_networks__network_ref__clusteroperations): Get cluster operations for an network.

??? example "Python API client: get queued cluster operations for an network"
    ...
    sf_client = apiclient.Client()
    md = sf_client.get_network_queued_cluster_operations(img_uuid)
    print(json.dumps(md, indent=4, sort_keys=True))
    ```

??? example "Python API client: get all cluster operations for an network"
    ...
    sf_client = apiclient.Client()
    md = sf_client.get_network_queued_cluster_operations(img_uuid, all=True)
    print(json.dumps(md, indent=4, sort_keys=True))
    ```