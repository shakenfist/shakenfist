...
become visible, not immediately — the other term is the resources
daemon's own publication cadence, which used to be the larger of the
two. It no longer is: `sf-resources` republishes within about five
seconds of the active-domain set changing
(`_should_publish_metrics()` in
`shakenfist/daemons/resources/main.py`) rather than waiting out its
metrics period. At cluster start-up, where no domains are yet running
for the set to change, that publish still waits out the once-a-minute
floor.
