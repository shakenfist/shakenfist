...
def generic_wrapper(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            msg = "API request: %s %s" % (flask.request.method, flask.request.url)
...
def arg_is_instance_uuid(func):
    ...
    def wrapper(*args, **kwargs):
        if "instance_uuid" in kwargs:
            kwargs["instance_from_db"] = db.get_instance(kwargs["instance_uuid"])
...
def arg_is_instance_uuid_as_virt(func):
    ...
    def wrapper(*args, **kwargs):
        if "instance_uuid" in kwargs:
            kwargs["instance_from_db_virt"] = virt.from_db(kwargs["instance_uuid"])
    ...
    def wrapper(*args, **kwargs):
        ...
        if i and i.db_entry["node"] != config.NODE_NAME:
            ...
            api_token = util.get_api_token(
                "http://%s:%d" % (i.db_entry["node"], config.get("API_PORT")),
            ...
            LOG.info(
                "Proxied %s %s returns: %d, %s"
                % (flask.request.environ["REQUEST_METHOD"], url, r.status_code, r.text)
            )
...
def requires_instance_ownership(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if get_jwt_identity() not in [
            kwargs["instance_from_db"]["namespace"],
            "system",
        ]:
            LOG.withField("instance", kwargs["instance_uuid"]).info(
    ...
    def wrapper(*args, **kwargs):
        if not util.is_network_node():
            admin_token = util.get_api_token(
                "http://%s:%d" % (config.NETWORK_NODE_IP, config.get("API_PORT")),
                namespace="system",
            )
            r = requests.request(
                flask.request.environ["REQUEST_METHOD"],
            ...
            LOG.info("Returning proxied request: %d, %s" % (r.status_code, r.text))
            resp = flask.Response(r.text, mimetype="application/json")
            resp.status_code = r.status_code
            return resp
...
def _metadata_putpost(meta_type, owner, key, value):
    ...
    with db.get_lock("metadata", meta_type, owner, op="Metadata update"):
...
class Root(Resource):
    def get(self):
        resp = flask.Response("Shaken Fist REST API service", mimetype="text/plain")
...
class Instances(Resource):
    ...
    def post(
        ...
        if not instance_uuid:
            ...
            db.add_event("instance", instance_uuid, "uuid allocated", None, None, None)
        ...
        if network:
            for netdesc in network:
                ...
                if not n:
                    ...
                    return error(404, "network %s not found" % netdesc["network_uuid"])
                ...
                with db.get_lock(
                    ...
                    if "address" not in netdesc or not netdesc["address"]:
                        ...
                        if not ipm.reserve(netdesc["address"]):
                            ...
                            db.enqueue_instance_error(instance_uuid, m)
                            return error(409, "address %s in use" % netdesc["address"])
        ...
        try:
            ...
            if not placed_on:
                ...
                SCHEDULER.place_instance(instance, network, candidates=[placed_on])
                placement = placed_on
        ...
        db.add_event("instance", instance_uuid, "placement", None, None, placement)
        ...
        db.add_event("instance", instance_uuid, "create", "enqueued", None, None)
    ...
    def delete(self, confirm=False, namespace=None):
        ...
        start_time = time.time()
        while waiting_for and (time.time() - start_time < config.get("API_ASYNC_WAIT")):
...
class InstanceSnapshot(Resource):
    ...
    def post(
        ...
        db.add_event(
            "instance", instance_uuid, "api", "snapshot (all=%s)" % all, None, snap_uuid
        )
...
class InstanceRebootSoft(Resource):
    ...
    @redirect_instance_request
    def post(
        self, instance_uuid=None, instance_from_db=None, instance_from_db_virt=None
    ):
        with db.get_lock(
            "instance",
            None,
            ...
            db.add_event("instance", instance_uuid, "api", "soft reboot", None, None)
...
class InstanceRebootHard(Resource):
    ...
    def post(
        ...
        with db.get_lock(
            ...
            db.add_event("instance", instance_uuid, "api", "hard reboot", None, None)
...
class InstancePowerOff(Resource):
    ...
    def post(
        ...
        with db.get_lock(
            ...
            db.add_event("instance", instance_uuid, "api", "poweroff", None, None)
...
class InstancePause(Resource):
    ...
    def post(
        ...
        with db.get_lock(
            "instance", None, instance_uuid, ttl=120, timeout=120, op="Instance pause"
        ):
...
class InstanceUnpause(Resource):
    ...
    def post(
        ...
        with db.get_lock(
            "instance", None, instance_uuid, ttl=120, timeout=120, op="Instance unpause"
        ):
...
def _safe_get_network_interface(interface_uuid):
    ...
    log = LOG.withFields(
        {"network": ni["network_uuid"], "networkinterface": ni["uuid"]}
    )
...
class InstanceMetadata(Resource):
    ...
    def delete(self, instance_uuid=None, key=None, instance_from_db=None):
        ...
        with db.get_lock(
            "metadata", "instance", instance_uuid, op="Instance metadata delete"
        ):
            md = db.get_metadata("instance", instance_uuid)
            if md is None or key not in md:
                return error(404, "key not found")
...
class InstanceConsoleData(Resource):
    ...
    def get(
        ...
        resp = flask.Response(
            instance_from_db_virt.get_console_data(length), mimetype="text/plain"
        )
        resp.status_code = 200
        return resp
...
class Images(Resource):
    ...
    def get(self, node=None):
        ...
        for db_name, meta in db_data.items():
            ...
            images.append({**meta, "ref": name_split[-2], "node": name_split[-1]})
        return images
...
def _delete_network(network_from_db):
    ...
    with db.get_object_lock(n, ttl=120, op="Network deleting"):
        ...
        if not n or n.is_dead():
            LOG.withFields(
                {"network_uuid": n.db_entry["uuid"], "state": n.db_entry["state"]}
...
class Networks(Resource):
    ...
    def post(
        ...
        network = db.allocate_network(
            netblock, provide_dhcp, provide_nat, name, namespace
        )
        ...
        db.add_event("network", network["uuid"], "deploy", "enqueued", None, None)
    ...
    def delete(self, confirm=False, namespace=None):
        ...
        if networks_unable:
            return error(403, {"deleted": networks_del, "unable": networks_unable})
...
class NetworkMetadata(Resource):
    ...
    def delete(self, network_uuid=None, key=None, network_from_db=None):
        ...
        with db.get_lock(
            "metadata", "network", network_uuid, op="Network metadata delete"
        ):
            md = db.get_metadata("network", network_uuid)
            if md is None or key not in md:
                return error(404, "key not found")
...
api.add_resource(AuthNamespaceKeys, "/auth/namespaces/<namespace>/keys")
api.add_resource(AuthNamespaceKey, "/auth/namespaces/<namespace>/keys/<key_name>")
api.add_resource(AuthMetadatas, "/auth/namespaces/<namespace>/metadata")
api.add_resource(AuthMetadata, "/auth/namespaces/<namespace>/metadata/<key>")
...
api.add_resource(InstanceMetadata, "/instances/<instance_uuid>/metadata/<key>")
api.add_resource(
    InstanceConsoleData,
...
api.add_resource(NetworkMetadata, "/networks/<network_uuid>/metadata/<key>")
