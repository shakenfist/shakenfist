...
class Namespace(dbo):
    ...
    def __init__(self, data) -> None:
        ...
        self.log = LOG.with_fields(
            {self.object_type: data.name})

        # Lazy-load attributes from MariaDB
        self.__attributes: Optional[
            NamespaceAttributesData] = None
    ...
    def uuid(self) -> str:
        ...
        This overrides the base class to return a string
        instead of uuid.UUID because namespaces use their
        name as their identifier for historical reasons.
        """
        return self._DatabaseBackedObject__uuid  # type: ignore

    def _load_attributes(
            self) -> Optional[NamespaceAttributesData]:
        """Load attributes from MariaDB."""
        if not self.__attributes_loaded:
            self.__attributes = \
                mariadb.get_namespace_attributes(self.uuid)
    ...
    def _ensure_attributes(self) -> NamespaceAttributesData:
        ...
        if attrs is None:
            ...
            if not mariadb.create_namespace_attributes(attrs):
                ...
                attrs = mariadb.get_namespace_attributes(
                    self.uuid)
    ...
    def _upgrade_step_2_to_3(cls, static_values):
        etcd.put(
            'attribute/namespace', static_values['uuid'],
            'trust', {'full_trust': ['system']})
    ...
    def _upgrade_step_3_to_4(cls, static_values):
        ...
        if keys:
            ...
            # Move across keys in the incorrect location.
            # These override as they are how the namespace
            # and auth code was actually using keys.
            for k in keys:
                nonced_keys[k] = {
                    'key': keys[k],
                    'nonce': sfrandom.random_id()
        ...
        if db_data:
            ...
            etcd.delete(
                'attribute/namespace',
                static_values['uuid'], 'service_key')

        etcd.put(
            'attribute/namespace', static_values['uuid'],
            'keys', {'nonced_keys': nonced_keys})

    @classmethod
    def _upgrade_step_4_to_5(cls, static_values):
        cls._upgrade_metadata_to_attribute(
            static_values['uuid'])
    ...
    def _upgrade_step_6_to_7(cls, static_values):
        # Migration to MariaDB namespaces/namespace_attributes
        # tables is handled by sf-ctl migrate-data-to-mariadb.
        ...

    @classmethod
    def _persist_pydantic_upgrade(  # type: ignore[override]
            cls, data: NamespaceData) -> None:
        """Persist an upgraded NamespaceData to MariaDB."""
        # Delete and recreate since we can't update the
        # primary key or version in-place easily.
    ...
    def _db_create(cls, object_uuid, metadata):
        ...
        add_event(
            EVENT_TYPE_AUDIT, cls.object_type, name,
            'db record created',
        ...
        if not data:
            if not suppress_failure_audit:
                add_event(
                    ...
                    extra={
                        'caller': util_callstack.get_caller(
                            offset=-3)
                    },
                    log_as_error=True)
            return None
    ...
    def new(cls, name):
        n = Namespace.from_db(
            name, suppress_failure_audit=True)
        if n:
            return n
    ...
    def remove_trust(self, namespace):
        with self.get_lock_attr('trust', 'Remove trust'):
            # You cannot remove the trust of the system
            # namespace, because if you could then the
            # cluster admin wouldn't see your resources.
            if namespace == 'system':
                return
    ...
    def external_view(self):
        # If this is an external view, then mix back in
        # attributes that users expect
...
def get_api_token(base_url, namespace='system'):
    ...
    # Cheat and don't actually call the auth API to create
    # a token, just call its underlying code, thus saving a
    # network round trip.
    token = access_tokens.create_token(
        ns, keyname, nonce, duration=5)

    CACHED_TOKENS[namespace] = (
        expiry, token['access_token'])
