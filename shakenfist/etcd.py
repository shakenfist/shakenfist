...
class ActualLock(Lock):
    def __init__(
        ...
        self._uuid = json.dumps(
            {"node": config.NODE_NAME, "pid": os.getpid(), "operation": self.operation},
            indent=4,
            sort_keys=True,
        )
        ...
        while time.time() - start_time < self.timeout:
            ...
            if res:
                ...
                if duration > threshold:
                    ...
                    self.log_ctx.withField("duration", duration).info(
                        "Acquiring a lock was slow"
                    )
                return self
...
def get_lock(objecttype, subtype, name, ttl=60, timeout=10, log_ctx=LOG, op=None):
...
def refresh_lock(lock, log_ctx=LOG):
    if not lock.is_acquired():
        log_ctx.withField("lock", lock.name).info("Attempt to refresh an expired lock")
        raise exceptions.LockException("The lock on %s has expired." % lock.path)
...
def dequeue(queuename):
    ...
    with get_lock("queue", None, queuename, op="Dequeue"):
        for data, metadata in client.get_prefix(
            queue_path, sort_order="ascend", sort_target="key"
        ):
