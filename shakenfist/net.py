...
class Network(object):
        ...
        if util.is_network_node():
            ...
            if not util.check_for_interface(subst["vx_veth_outer"]):
                with util.RecordedOperation("create router veth", self):
                    ...
                    util.execute(
                        None, "brctl addif %(vx_bridge)s %(vx_veth_outer)s" % subst
                    )
                    util.execute(None, "ip link set %(vx_veth_outer)s up" % subst)
                    util.execute(
                        None, "%(in_netns)s ip link set %(vx_veth_inner)s up" % subst
                    )
                    util.execute(
                        None,
            ...
            if not util.check_for_interface(subst["physical_veth_outer"]):
                with util.RecordedOperation("create physical veth", self):
                    ...
                    util.execute(None, "ip link set %(physical_veth_outer)s up" % subst)
                    util.execute(
                        None,
            ...
            db.add_event(
                "network", self.db_entry["uuid"], "deploy", "enqueued", None, None
            )
    ...
    def deploy_nat(self):
        ...
        if not self.db_entry["floating_gateway"]:
            with db.get_lock(
                "ipmanager", None, "floating", ttl=120, op="Network deploy NAT"
            ):
        ...
        with db.get_object_lock(self, ttl=120, op="Network deploy NAT"):
            ...
            if not util.nat_rules_for_ipblock(self.network_address):
                with util.RecordedOperation("enable nat", self):
                    util.execute(None, "echo 1 > /proc/sys/net/ipv4/ip_forward")
                    util.execute(
                        None,
        ...
        with db.get_object_lock(self, ttl=120, op="Network delete"):
            ...
            if util.check_for_interface(subst["vx_interface"]):
                with util.RecordedOperation("delete vxlan interface", self):
                    util.execute(None, "ip link delete %(vx_interface)s" % subst)
            ...
            if util.is_network_node():
                if util.check_for_interface(subst["vx_veth_outer"]):
                    with util.RecordedOperation("delete router veth", self):
                        util.execute(None, "ip link delete %(vx_veth_outer)s" % subst)
    ...
    def update_dhcp(self):
        ...
        if util.is_network_node():
            ...
            db.enqueue("networknode", UpdateDHCPNetworkTask(self.db_entry["uuid"]))
            db.add_event(
                "network", self.db_entry["uuid"], "update dhcp", "enqueued", None, None
            )
    ...
    def remove_dhcp(self):
        if util.is_network_node():
            ...
            db.enqueue("networknode", RemoveDHCPNetworkTask(self.db_entry["uuid"]))
            db.add_event(
                "network", self.db_entry["uuid"], "remove dhcp", "enqueued", None, None
            )
    ...
    def ensure_mesh(self):
        with db.get_object_lock(self, ttl=120, op="Network ensure mesh"):
            ...
            LOG.withObj(self).withField("discovered", discovered).debug(
                "Discovered mesh elements"
            )
    ...
    def add_floating_ip(self, floating_address, inner_address):
        LOG.withObj(self).info(
            "Adding floating ip %s -> %s" % (floating_address, inner_address)
        )
        subst = self.subst_dict()
    ...
    def remove_floating_ip(self, floating_address, inner_address):
        LOG.withObj(self).info(
            "Removing floating ip %s -> %s" % (floating_address, inner_address)
        )
        subst = self.subst_dict()
        subst["floating_address"] = floating_address
        subst["inner_address"] = inner_address
