...
def _get_cache_path():
    ...
    if not os.path.exists(image_cache_path):
        LOG.withField("image_cache_path", image_cache_path).debug(
...
class Image(object):
    ...
    def from_url(url, checksum=None):
        ...
        db_data = db.get_image_metadata(Image.calc_unique_ref(url), config.NODE_NAME)
    ...
    def persist(self):
        ...
        db.persist_image_metadata(self.unique_ref, config.NODE_NAME, metadata)
    ...
    def _fetch(self, resp, locks=None):
        ...
        if fetched > 0:
            self.persist()
            LOG.withImage(self).withField("bytes_fetched", fetched).info(
                "Fetch complete"
            )
    ...
    def correct_checksum(self, image_name):
        ...
        log.withField("checksum", md5_hash.hexdigest()).debug(
            "Calc from image download"
        )
...
def create_cow(locks, cache_file, disk_file, disk_size):
    ...
    util.execute(
        locks,
        "qemu-img create -b %s -f qcow2 %s %dG" % (cache_file, disk_file, disk_size),
    )
...
def create_raw(locks, cache_file, disk_file):
    ...
    util.execute(
        locks, "qemu-img convert -t none -O raw %s %s" % (cache_file, disk_file)
    )
...
def snapshot(locks, source, destination):
    ...
    util.execute(
        locks, "qemu-img convert --force-share -O qcow2 %s %s" % (source, destination)
    )
