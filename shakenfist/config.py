...
class SFConfig(BaseSettings):
    ...
    API_VALIDATION_MODE: Literal['off', 'warn', 'enforce'] = Field(
        ...
        description=(
            ...
            'shape, naming the offending parameter; since phase 6 that '
            'includes missing-required findings, so a parameter '
            'declared required but omitted or sent as an explicit null '
            'is refused too; "warn" logs what it would have rejected and '
