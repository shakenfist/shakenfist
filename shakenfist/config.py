...
class SFConfig(BaseSettings):
    ...
    API_VALIDATION_MODE: Literal['off', 'warn', 'enforce'] = Field(
        ...
        description=(
            ...
            'shape, naming the offending parameter, for every finding -- '
            'a missing-required one included, so a parameter declared '
            'required but omitted, or sent as an explicit null, is '
            'refused too; "warn" records what it would have '
            'rejected and leaves the response to the handler; "off" '
            'disables the layer entirely. "warn" and "off" are the '
            'rollback for an operator whose callers send input the '
            'published declarations do not describe. Neither restores the '
            'pre-validation behaviour exactly: an undeclared body key '
            'answers 500 rather than 400, and the handler guards refuse in '
            'every mode -- see '
