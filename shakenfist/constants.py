...
# Bounds on a per-object events read. Enforced by the REST API layer
# and by both mariadb read paths, and used to build the published
# swagger description, so the contract cannot drift from what is
# enforced. Here rather than in either module because base.py already
# imports mariadb, so mariadb importing base would be circular.
