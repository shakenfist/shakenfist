...
def get_lock(
    ...
    return etcd.get_lock(
        objecttype, subtype, name, ttl=ttl, timeout=timeout, log_ctx=log_ctx, op=None
    )


def get_object_lock(
    obj, ttl=60, timeout=ETCD_ATTEMPT_TIMEOUT, relatedobjects=None, log_ctx=LOG, op=None
):
    obj_type, obj_name = obj.unique_label()
    if not (obj_type and obj_name):
        raise exceptions.LockException("Could not derive lock name from %s" % obj)
    return get_lock(
        obj_type,
        None,
        obj_name,
...
def allocate_network(
    netblock, provide_dhcp=True, provide_nat=False, name=None, namespace=None
):
...
def update_instance_power_state(instance_uuid, state):
    ...
    if (
        i.get("power_state", "").startswith("transition-to-")
        and i.get("power_state_previous") == state
        and state_age < 70
    ):
        return
...
def update_instance_error_message(instance_uuid, error_message):
    ...
    add_event("instance", instance_uuid, "error message", error_message, None, None)
...
def get_events(object_type, object_uuid):
    for m in etcd.get_all("event/%s" % object_type, object_uuid, sort_order="ascend"):
        yield m
...
def update_metrics_bulk(metrics):
    etcd.put(
        ...
        config.NODE_NAME,
        None,
        {"fqdn": config.NODE_NAME, "timestamp": time.time(), "metrics": metrics},
        ttl=120,
    )
...
def enqueue_instance_delete_remote(node, instance_uuid):
    enqueue(
        node,
        {
            "tasks": [DeleteInstanceTask(instance_uuid)],
        },
    )
...
def enqueue_instance_error(instance_uuid, error_msg):
    enqueue(
        config.NODE_NAME,
        {
            "tasks": [ErrorInstanceTask(instance_uuid, error_msg)],
        },
    )
