    ...
    try:
        for task in workitem.get("tasks", []):
            if not QueueTask.__subclasscheck__(type(task)):
                raise exceptions.UnknownTaskException("Task was not decoded: %s" % task)

            if InstanceTask.__subclasscheck__(type(task)) or isinstance(
                task, FetchImageTask
            ):
        ...
        if instance_uuid:
            db.enqueue_instance_error(instance_uuid, "failed queue task: %s" % e)
...
def image_fetch(url, instance_uuid):
    ...
    try:
        ...
        if instance_uuid:
            db.enqueue_instance_error(instance_uuid, "Image fetch failed: %s" % e)
        ...
        raise exceptions.ImageFetchTaskFailedException("Failed to fetch image %s" % url)
...
def instance_preflight(instance_uuid, network):
    ...
    try:
        ...
        raise exceptions.AbortInstanceStartException("Unable to find suitable node")
...
def instance_start(instance_uuid, network):
    ...
    with db.get_lock(
        "instance", None, instance_uuid, ttl=900, timeout=120, op="Instance start"
    ) as lock:
        instance = virt.from_db(instance_uuid)
        ...
        try:
            ...
            if code in (libvirt.VIR_ERR_CONFIG_UNSUPPORTED, libvirt.VIR_ERR_XML_ERROR):
                db.enqueue_instance_error(
                    instance_uuid, "instance failed to start: %s" % e
                )
                return
...
def instance_delete(instance_uuid):
    with db.get_lock(
        "instance", None, instance_uuid, timeout=120, op="Instance delete"
    ):
            ...
            if n:
                ...
                if network in host_networks:
                    with util.RecordedOperation(
                        "deallocate ip address", instance_from_db_virt
                    ):
                        n.update_dhcp()
                else:
