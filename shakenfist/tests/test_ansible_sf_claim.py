...
class SfClaimMakeClientTestCase(base.ShakenFistTestCase):
    """A partial connection set fails the module rather than being dropped.

    api_url, auth_namespace and key are supplied together or not at all.
    Falling back to discovery when only some of them arrive would build a
    client pointed at whatever cloud the environment, ~/.shakenfist or
    /etc/sf/shakenfist.json names, rather than the one the playbook asked
    for, and say nothing about it -- a playbook templating these from
    partially populated inventory is exactly how that happens.
    """

    def _kwargs(self, **params):
        with mock.patch.object(sf_claim.apiclient, 'Client') as client:
            sf_claim._make_client(FakeModule(**params))
        return client.call_args[1]

    def test_all_three_are_used_verbatim(self):
        kwargs = self._kwargs(api_url='https://api.example.com',
                              auth_namespace='ns', key='k')
    ...
    def test_a_partial_set_fails_the_module(self):
        ...
        for params in [{'auth_namespace': 'ns', 'key': 'k'},
                       {'api_url': 'https://api.example.com', 'key': 'k'},
                       {'api_url': 'https://api.example.com',
                        'auth_namespace': 'ns'}]:
            with mock.patch.object(sf_claim.apiclient, 'Client') as client:
                failure = self.assertRaises(
                    ModuleFailed, sf_claim._make_client, FakeModule(**params))
            ...
            detail = failure.result['msg'].split('Got only ', 1)[1]
            for name in ('api_url', 'auth_namespace', 'key'):
                if name in params:
