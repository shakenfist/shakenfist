...
class AuthTestCase(test_shakenfist.ShakenFistTestCase):
    ...
    def test_post_auth_no_args(self):
        ...
        self.assertEqual(
            {"error": "missing namespace in request", "status": 400}, resp.get_json()
        )

    def test_post_auth_no_key(self):
        resp = self.client.post("/auth", data=json.dumps({"namespace": "banana"}))
        self.assertEqual(400, resp.status_code)
        self.assertEqual(
            {"error": "missing key in request", "status": 400}, resp.get_json()
        )
    ...
    def test_post_auth_key_non_string(self):
        ...
        self.assertEqual(400, resp.status_code)
        self.assertEqual(
            {"error": "key is not a string", "status": 400}, resp.get_json()
        )
    ...
    def test_post_auth_not_authorized(self, mock_get_keys):
        ...
        self.assertEqual({"error": "unauthorized", "status": 401}, resp.get_json())
    ...
    def test_no_auth_header(self):
        resp = self.client.post(
            "/auth/namespaces", data=json.dumps({"namespace": "foo"})
        )
        self.assertEqual(401, resp.status_code)
        self.assertEqual(
    ...
    def test_auth_header_bad_jwt(self):
        ...
        self.assertEqual(
            {"error": "invalid JWT in Authorization header", "status": 401},
            _clean_traceback(resp.get_json()),
        )
        self.assertEqual(401, resp.status_code)
...
class ExternalApiTestCase(test_shakenfist.ShakenFistTestCase):
    def setUp(self):
        ...
        self.get_keys = mock.patch(
            "shakenfist.external_api.app.Auth._get_keys", return_value=("foo", ["bar"])
        )
...
class ExternalApiGeneralTestCase(ExternalApiTestCase):
    ...
    def test_get_root(self):
        ...
        self.assertEqual(
            "Shaken Fist REST API service", resp.get_data().decode("utf-8")
        )
        self.assertEqual(200, resp.status_code)
    ...
    def test_auth_add_key_missing_args(self):
        ...
        self.assertEqual(400, resp.status_code)
        self.assertEqual(
            {"error": "no namespace specified", "status": 400}, resp.get_json()
        )
    ...
    def test_auth_add_key_missing_key(self, mock_put, mock_get, mock_lock):
        resp = self.client.post(
            "/auth/namespaces",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"namespace": "foo", "key_name": "bernard"}),
        )
        self.assertEqual(400, resp.status_code)
        self.assertEqual({"error": "no key specified", "status": 400}, resp.get_json())
    ...
    def test_auth_add_key_illegal_keyname(self, mock_get, mock_lock):
        resp = self.client.post(
            ...
            headers={"Authorization": self.auth_header},
            data=json.dumps(
                {"namespace": "foo", "key_name": "service_key", "key": "cheese"}
            ),
        )
        self.assertEqual({"error": "illegal key name", "status": 403}, resp.get_json())
    ...
    def test_auth_add_key_new_namespace(
        self, mock_hashpw, mock_put, mock_get, mock_lock
    ):
        resp = self.client.post(
            "/auth/namespaces",
            headers={"Authorization": self.auth_header},
            data=json.dumps(
                {"namespace": "foo", "key_name": "bernard", "key": "cheese"}
            ),
        )
        self.assertEqual(200, resp.status_code)
        self.assertEqual("foo", resp.get_json())
    ...
    @mock.patch(
        "shakenfist.etcd.get_all",
        return_value=[{"name": "aaa"}, {"name": "bbb"}, {"name": "ccc"}],
    )
    def test_get_namespaces(self, mock_get_all):
        resp = self.client.get(
            "/auth/namespaces", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(200, resp.status_code)
    ...
    def test_delete_namespace_system(self):
        resp = self.client.delete(
            "/auth/namespaces/system", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(403, resp.status_code)
        self.assertEqual(
            {"error": "you cannot delete the system namespace", "status": 403},
            resp.get_json(),
        )
    ...
    def test_delete_namespace_with_instances(self, mock_get_instances):
        resp = self.client.delete(
            "/auth/namespaces/foo", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(400, resp.status_code)
        self.assertEqual(
            {"error": "you cannot delete a namespace with instances", "status": 400},
            resp.get_json(),
        )

    @mock.patch("shakenfist.db.get_instances", return_value=[])
    @mock.patch(
        "shakenfist.db.get_networks", return_value=[{"uuid": "123", "state": "created"}]
    )
    def test_delete_namespace_with_networks(
        self, mock_get_networks, mock_get_instances
    ):
        resp = self.client.delete(
            "/auth/namespaces/foo", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(400, resp.status_code)
        self.assertEqual(
            {"error": "you cannot delete a namespace with networks", "status": 400},
            resp.get_json(),
        )
    ...
    def test_delete_namespace_key_missing_key(self, mock_get, mock_lock):
        resp = self.client.delete(
            "/auth/namespaces/system/keys/mykey",
            headers={"Authorization": self.auth_header},
        )
        self.assertEqual(404, resp.status_code)
        self.assertEqual(
            {"error": "key name not found in namespace", "status": 404}, resp.get_json()
        )
    ...
    @mock.patch("shakenfist.db.get_lock")
    def test_put_namespace_metadata(self, mock_get_lock, mock_md_put, mock_md_get):
        resp = self.client.put(
            "/auth/namespaces/foo/metadata/foo",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
        self.assertEqual(200, resp.status_code)
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_post_namespace_metadata(self, mock_get_lock, mock_md_put, mock_md_get):
        resp = self.client.post(
            "/auth/namespaces/foo/metadata",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch(
        "shakenfist.db.get_metadata", return_value={"foo": "bar", "real": "smart"}
    )
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_namespace_metadata(self, mock_get_lock, mock_md_put, mock_md_get):
        resp = self.client.delete(
            "/auth/namespaces/foo/metadata/foo",
            headers={"Authorization": self.auth_header},
        )
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_namespace_metadata_bad_key(
        self, mock_get_lock, mock_md_put, mock_md_get
    ):
        resp = self.client.delete(
            "/auth/namespaces/foo/metadata/wrong",
            headers={"Authorization": self.auth_header},
        )
        self.assertEqual({"error": "key not found", "status": 404}, resp.get_json())
        self.assertEqual(404, resp.status_code)
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_namespace_metadata_no_keys(
        self, mock_get_lock, mock_md_put, mock_md_get
    ):
        resp = self.client.delete(
            "/auth/namespaces/foo/metadata/wrong",
    ...
    @mock.patch(
        "shakenfist.db.get_instance",
        return_value={"uuid": "123", "name": "banana", "namespace": "foo"},
    )
    def test_get_instance(self, mock_get_instance):
        ...
        self.assertEqual(
            {"uuid": "123", "name": "banana", "namespace": "foo"}, resp.get_json()
        )
        self.assertEqual(200, resp.status_code)
        self.assertEqual("application/json", resp.content_type)
    ...
    def test_get_instance_not_found(self, mock_get_instance):
        ...
        self.assertEqual(
            {"error": "instance not found", "status": 404}, resp.get_json()
        )
        self.assertEqual(404, resp.status_code)
        self.assertEqual("application/json", resp.content_type)
    ...
    @mock.patch(
        "shakenfist.db.get_instance",
        return_value={"uuid": "foo", "name": "banana", "namespace": "foo"},
    )
    @mock.patch("shakenfist.db.get_metadata", return_value={"a": "a", "b": "b"})
    ...
    def test_put_instance_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_instance
    ):
        resp = self.client.put(
            "/instances/foo/metadata/foo",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_post_instance_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_instance
    ):
        resp = self.client.post(
            "/instances/foo/metadata",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_put_network_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_network
    ):
        resp = self.client.put(
            "/networks/foo/metadata/foo",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_post_network_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_network
    ):
        resp = self.client.post(
            "/networks/foo/metadata",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"key": "foo", "value": "bar"}),
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_instance_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_instance
    ):
        resp = self.client.delete(
            "/instances/foo/metadata/foo", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(None, resp.get_json())
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_instance_metadata_bad_key(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_instance
    ):
        resp = self.client.delete(
            "/instances/foo/metadata/wrong", headers={"Authorization": self.auth_header}
        )
        self.assertEqual({"error": "key not found", "status": 404}, resp.get_json())
        self.assertEqual(404, resp.status_code)
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_network_metadata(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_network
    ):
        resp = self.client.delete(
            "/networks/foo/metadata/foo", headers={"Authorization": self.auth_header}
        )
        self.assertEqual(None, resp.get_json())
        self.assertEqual(200, resp.status_code)
    ...
    @mock.patch("shakenfist.db.persist_metadata")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_network_metadata_bad_key(
        self, mock_get_lock, mock_md_put, mock_md_get, mock_get_network
    ):
        resp = self.client.delete(
            "/networks/foo/metadata/wrong", headers={"Authorization": self.auth_header}
        )
        self.assertEqual({"error": "key not found", "status": 404}, resp.get_json())
        self.assertEqual(404, resp.status_code)

    @mock.patch(
        "shakenfist.db.get_nodes",
        return_value=[
            {
                "fqdn": "sf-1",
                "ip": "192.168.72.240",
    ...
    def test_get_node(self, mock_md_get):
        resp = self.client.get("/nodes", headers={"Authorization": self.auth_header})
        self.assertEqual(
            [
                {
                    "name": "sf-1",
...
class ExternalApiInstanceTestCase(ExternalApiTestCase):
    def setUp(self):
        ...
        self.virt_from_db = mock.patch("shakenfist.virt.from_db", fake_virt_from_db)
        ...
        self.config = mock.patch("shakenfist.configuration.config", fake_config)
    ...
    def test_delete_all_instances(
        ...
        resp = self.client.delete(
            "/instances",
            headers={"Authorization": self.auth_header},
            data=json.dumps({"confirm": True, "namespace": "foo"}),
        )
        self.assertEqual(
            [
                "6a973b82-31b3-4780-93e4-04d99ae49f3f",
    ...
    def test_delete_all_instances_one_already_deleted(
        ...
        self.assertEqual(["847b0327-9b17-4148-b4ed-be72b6722c17"], resp.get_json())
        self.assertEqual(200, resp.status_code)
            ...
            data=json.dumps(
                {
                    ...
                    "cpus": 1,
                    "memory": 1024,
                    "network": [{"uuid": "87c15186-5f73-4947-a9fb-2183c4951efc"}],
                    "disk": [{"size": 8, "base": "cirros"}],
                    "ssh_key": None,
                    "user_data": None,
                    "placed_on": None,
    ...
    def test_post_instance_only_system_allocates_uuids(self):
        ...
        resp = self.client.post(
            "/instances",
            headers={"Authorization": non_system_auth_header},
            data=json.dumps(
                {
                    "name": "test_instance",
                    "cpus": 1,
...
class ExternalApiNetworkTestCase(ExternalApiTestCase):
    def setUp(self):
        ...
        self.config = mock.patch("shakenfist.configuration.config", fake_config_network)
        self.mock_config = self.config.start()
    ...
    @mock.patch(
        "shakenfist.db.get_ipmanager", return_value=ipmanager.NetBlock("10.0.0.0/24")
    )
    ...
    @mock.patch("shakenfist.etcd.put")
    @mock.patch("shakenfist.db.get_lock")
    def test_delete_all_networks_none_to_delete(
        self,
        mock_db_get_lock,
        mock_etcd_put,
        mock_remove_dhcp,
