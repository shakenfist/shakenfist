...
class ActualLockTestCase(test_shakenfist.ShakenFistTestCase):
    def setUp(self):
        ...
        self.config = mock.patch("shakenfist.etcd.config", fake_config)
    ...
    def test_context_manager(self, mock_pid, mock_acquire, mock_release):
        ...
        self.assertEqual(
            json.dumps(
                {"node": "thisnode", "pid": 42, "operation": "Test case"},
                indent=4,
                sort_keys=True,
            ),
            al._uuid,
    ...
    @mock.patch("shakenfist.etcd.ActualLock.get_holder", return_value=("foo", "43"))
    ...
    def test_context_manager_timeout(
        ...
        al = etcd.ActualLock("instance", None, "auuid", op="Test case", timeout=4)
        al.log_ctx = mock.MagicMock()
...
class TaskEncodingETCDtestCase(test_shakenfist.ShakenFistTestCase):
    ...
    def test_put_PreflightInstanceTask(self, mock_put):
        etcd.put(
            "objecttype", "subtype", "name", tasks.PreflightInstanceTask("fake_uuid")
        )
    ...
    def test_put_DeleteInstanceTask(self, mock_put):
        etcd.put("objecttype", "subtype", "name", tasks.DeleteInstanceTask("fake_uuid"))
    ...
    def test_put_DeployNetworkTask(self, mock_put):
        etcd.put("objecttype", "subtype", "name", tasks.DeployNetworkTask("fake_uuid"))
    ...
    def test_put_FetchImageTask(self, mock_put):
        etcd.put(
            "objecttype", "subtype", "name", tasks.FetchImageTask("http://server/image")
        )
    ...
    "version": 1
        ...
        etcd.put(
            ...
            "subtype",
            "name",
            tasks.FetchImageTask("http://server/image", instance_uuid="fake_uuid"),
        )
...
class TaskDequeueTestCase(test_shakenfist.ShakenFistTestCase):
    @mock.patch(
        "etcd3gw.Etcd3Client.get_prefix",
        return_value=[
            (
                """{
            "tasks": [
