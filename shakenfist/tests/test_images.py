...
class ImageResolversTestCase(test_shakenfist.ShakenFistTestCase):
    ...
    def test_resolve_cirros(self, mock_get):
        ...
        self.assertRaises(
            exceptions.VersionSpecificationError,
            image_resolver_cirros.resolve,
            "cirros***",
        )

    @mock.patch("requests.get", return_value=FakeResponse(404, None))
    def test_resolve_cirros_error(self, mock_get):
        self.assertRaises(exceptions.HTTPError, image_resolver_cirros.resolve, "cirros")
    ...
    @mock.patch(
        "shakenfist.image_resolver_ubuntu.UBUNTU_URL", "https://cloud-images.ubuntu.com"
    )
    def test_resolve_ubuntu(self, mock_get):
        ...
        self.assertRaises(
            exceptions.VersionSpecificationError,
            image_resolver_ubuntu.resolve,
            "ubuntu***",
        )

    @mock.patch("requests.get", return_value=FakeResponse(404, None))
    def test_resolve_ubuntu_error(self, mock_get):
        self.assertRaises(exceptions.HTTPError, image_resolver_ubuntu.resolve, "ubuntu")
...
class ImageObjectTestCase(test_shakenfist.ShakenFistTestCase):
    ...
    @mock.patch(
        "shakenfist.image_resolver_cirros.resolve",
        return_value=("!!!cirros!!!", "123abc"),
    )
    @mock.patch(
        "shakenfist.image_resolver_ubuntu.resolve",
    ...
    def test_hash_image(self, mock_mkdirs, mock_get_meta):
        ...
        self.assertEqual(
            "f0e6a6a97042a4f1f1c87f5f7d44315b2d" "852c2df5c7991cc66241bf7072d1c4",
            img.unique_ref,
        )
    ...
    @mock.patch(
        "shakenfist.db.get_image_metadata",
        return_value={
            "checksum": None,
    ...
    def test_does_not_require_fetch(
        self, mock_mkdirs, mock_get_meta, mock_request_head
    ):
        img = images.Image.from_url("http://example.com")
    ...
    def test_version_image_path(self, mock_mkdirs, mock_get_meta):
        ...
        img.file_version = 1
        self.assertEqual(
            "/a/b/c/image_cache/bbf155338660b476435"
    ...
    @mock.patch(
        ...
        return_value=FakeResponse(
            ...
            "",
            headers={
                "Last-Modified": "Fri, 06 Mar 2020 19:19:05 GMT",
    ...
    def test_fetch_image_new(self, mock_get, mock_makedirs, mock_get_meta):
        img = images.Image.from_url("http://example.com")
        dirty_fields = img._new_image_available(img._open_connection())
        self.assertEqual(
            ("modified", None, "Tue, 10 Sep 2019 07:24:40 GMT"), dirty_fields
        )
    ...
    def test_transcode_image_noop(
        self, mock_event, mock_lock, mock_exists, mock_execute
    ):
    ...
    @mock.patch("shakenfist.util.execute", return_value=(None, None))
    @mock.patch("os.path.exists", return_value=False)
    @mock.patch("shakenfist.images.identify", return_value={"file format": "qcow2"})
    @mock.patch("os.link")
    @mock.patch("shakenfist.db.get_lock")
    @mock.patch("shakenfist.db.add_event")
    def test_transcode_image_link(
        self, mock_event, mock_lock, mock_link, mock_identify, mock_exists, mock_execute
    ):
        images._transcode(None, "/a/b/c/hash", FakeImage())
    ...
    @mock.patch("shakenfist.util.execute", return_value=(None, None))
    @mock.patch("os.path.exists", return_value=False)
    @mock.patch("shakenfist.images.identify", return_value={"file format": "raw"})
    @mock.patch("os.link")
    @mock.patch("shakenfist.db.get_lock")
    @mock.patch("shakenfist.db.add_event")
    def test_transcode_image_convert(
        self, mock_event, mock_lock, mock_link, mock_identify, mock_exists, mock_execute
    ):
        images._transcode(None, "/a/b/c/hash", FakeImage())
    ...
    @mock.patch("shakenfist.util.execute", return_value=(QEMU_IMG_OUT, None))
    @mock.patch("os.path.exists", return_value=True)
