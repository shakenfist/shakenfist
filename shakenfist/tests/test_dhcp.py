...
class DHCPTestCase(testtools.TestCase):
    def setUp(self):
        ...
        self.config = mock.patch("shakenfist.dhcp.config", fake_config)
        ...
        self.template = mock.patch(
            "shakenfist.dhcp.DHCP._read_template", fake_read_template
        )
    ...
    @mock.patch("os.path.exists", return_value=True)
    @mock.patch(
        "shakenfist.db.get_network_interfaces",
        return_value=[
            {
    ...
    @mock.patch(
        ...
        side_effect=[
            {"uuid": "instuuid1", "name": "inst1"},
            {"uuid": "instuuid2", "name": "in,,,st2"},
        ],
    )
    ...
    @mock.patch("os.kill")
    def test_send_signal_no_process(self, mock_kill, mock_pid_exists, mock_path_exists):
        d = dhcp.DHCP(FakeNetwork(), "eth0")
    ...
    def test_restart_dhcpd(self, mock_execute, mock_hosts, mock_config, mock_signal):
        d = dhcp.DHCP(FakeNetwork(), "eth0")
        d.restart_dhcpd()
