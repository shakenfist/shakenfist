...
class MigrateStatesToMariadbTestCase(base.ShakenFistTestCase):
    """Tests for migrate_state_to_mariadb command."""
    ...
    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_dry_run_no_objects(self, mock_etcd, mock_mariadb):
        from shakenfist.client.ctl import migrate_state_to_mariadb

        mock_etcd.get_all.return_value = []

        result = self.runner.invoke(migrate_state_to_mariadb, ['--dry-run'])

        self.assertEqual(result.exit_code, 0)
        mock_mariadb.ensure_schema.assert_not_called()
        self.assertIn('dry run', result.output.lower())

    @mock.patch('shakenfist.client.ctl.State')
    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_migrate_one_object(self, mock_etcd, mock_mariadb, mock_state):
        from shakenfist.client.ctl import migrate_state_to_mariadb

        test_uuid = str(uuid.uuid4())

        def get_all_side_effect(object_type, _):
            if object_type == 'instance':
                return [(f'/sf/object/instance/{test_uuid}', {})]
            return []

        mock_etcd.get_all.side_effect = get_all_side_effect
        mock_etcd.get.return_value = {'value': 'created', 'version': 1}

        result = self.runner.invoke(migrate_state_to_mariadb, [])

        self.assertEqual(result.exit_code, 0)
        mock_mariadb.ensure_schema.assert_called_once()
        mock_mariadb.set_state.assert_called()


class MigrateReferencesToMariadbTestCase(base.ShakenFistTestCase):
    """Tests for migrate_references_to_mariadb command."""
    ...
    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_dry_run_no_instances(self, mock_etcd, mock_mariadb):
        from shakenfist.client.ctl import migrate_references_to_mariadb

        mock_etcd.get_all.return_value = []
        mock_etcd.get.return_value = None

        result = self.runner.invoke(migrate_references_to_mariadb, ['--dry-run'])

        self.assertEqual(result.exit_code, 0)
        mock_mariadb.record_relationship.assert_not_called()
        self.assertIn('dry run', result.output.lower())

    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_migrate_instance_disk_reference(self, mock_etcd, mock_mariadb):
        from shakenfist.client.ctl import migrate_references_to_mariadb

        instance_uuid = str(uuid.uuid4())
        blob_uuid = str(uuid.uuid4())

        def get_all_side_effect(object_type, _uuid, **kwargs):
            if object_type == 'instance':
                return [(
                    f'/sf/object/instance/{instance_uuid}',
                    {'disk_spec': [{'blob_uuid': blob_uuid}]}
                )]
            return []

        mock_etcd.get_all.side_effect = get_all_side_effect
        mock_etcd.get.return_value = None
        mock_mariadb.record_relationship.return_value = True

        result = self.runner.invoke(migrate_references_to_mariadb, [])

        self.assertEqual(result.exit_code, 0)
        mock_mariadb.record_relationship.assert_called()

    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_dry_run_shows_would_create(self, mock_etcd, mock_mariadb):
        from shakenfist.client.ctl import migrate_references_to_mariadb
        ...
        self.assertEqual(result.exit_code, 0)
        self.assertIn('Would create DISK reference', result.output)
        mock_mariadb.record_relationship.assert_not_called()

    @mock.patch('shakenfist.client.ctl.mariadb')
    @mock.patch('shakenfist.client.ctl.etcd')
    def test_invalid_uuid_records_error(self, mock_etcd, mock_mariadb):
        from shakenfist.client.ctl import migrate_references_to_mariadb

        def get_all_side_effect(object_type, _uuid, **kwargs):
            if object_type == 'instance':
                return [(
                    '/sf/object/instance/not-a-valid-uuid',
                    {'disk_spec': []}
                )]
            return []
        ...
        self.assertEqual(result.exit_code, 0)
        self.assertIn('Invalid instance UUID', result.output)
