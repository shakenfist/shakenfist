...
def fake_get(objecttype, subtype, name):
    ...
    return FAKE_ETCD_STATE.get(
        "%s/%s/%s" % (objecttype, subtype, name), {"uuid": name, "node": "abigcomputer"}
    )
...
class CleanerTestCase(test_shakenfist.ShakenFistTestCase):
    def setUp(self):
        ...
        self.libvirt = mock.patch(
            "shakenfist.util.get_libvirt", return_value=FakeLibvirt()
        )
        ...
        self.config = mock.patch("shakenfist.daemons.cleaner.config", fake_config)
    ...
    def test_update_power_states(
        self, mock_time, mock_exists, mock_put, mock_get, mock_event, mock_see
    ):
