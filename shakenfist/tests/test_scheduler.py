...
class SchedulerTestCase(test_shakenfist.ShakenFistTestCase):
    def setUp(self):
        ...
        self.recorded_op = mock.patch("shakenfist.util.RecordedOperation")
...
class LowResourceTestCase(SchedulerTestCase):
    ...
    def setUp(self):
        ...
        mock_db_get_nodes = mock.patch(
            "shakenfist.db.get_nodes", side_effect=self.fake_db.get_nodes
        )
        mock_db_get_nodes.start()
        self.addCleanup(mock_db_get_nodes.stop)

        mock_db_get_metrics = mock.patch(
            "shakenfist.db.get_metrics", side_effect=self.fake_db.get_metrics
        )
    ...
    def test_not_enough_ram_for_system(self):
        ...
        fake_inst.db_setup(
            cpus=1,
            memory=1024,
            block_devices={"devices": [{"size": 21, "base": "some-os"}]},
        )
    ...
    def test_ok(self, mock_get_image_meta):
        ...
        fake_inst.db_setup(
            cpus=1,
            memory=1024,
            block_devices={"devices": [{"size": 8, "base": "some-os"}]},
        )
...
class CorrectAllocationTestCase(SchedulerTestCase):
    ...
    def setUp(self):
        ...
        self.mock_get_instances = mock.patch(
            "shakenfist.db.get_instances", side_effect=self.fake_db.get_instances
        )
        self.mock_get_instances.start()
        self.addCleanup(self.mock_get_instances.stop)
...
class FindMostTestCase(SchedulerTestCase):
    ...
    @mock.patch(
        "shakenfist.db.get_image_metadata_all",
        return_value={
