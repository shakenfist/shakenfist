...
from shakenfist.etcd_schema.operations.net_iface_ip_op import create_and_enqueue
from shakenfist.etcd_schema.operations.net_iface_ip_op import current_version
from shakenfist.etcd_schema.operations.net_iface_ip_op import model
from shakenfist.etcd_schema.operations.net_iface_ip_op import model_tasks
...
class NetIfaceIPOpTestCase(base.ShakenFistTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_model(self):
        ...
        self.assertEqual(['interface_defloat'], serialized['tasks'])
    ...
    def test_create_and_enqueue(self, _mock_time, _mock_id):
        ...
        self.assertEqual(CLUSTER_OPERATIONS.net_iface_ip_op, op_type)
        ...
        self.assertEqual(
            ...
            self.mock_etcd.get_raw(f'/sf/net_iface_ip_op/{op_uuid}')
        )
        self.assertEqual(
            {
                'value': 'queued',
            ...
            self.mock_etcd.get_raw(
                f'/sf/attribute/net_iface_ip_op/{op_uuid}/state')
        )
        self.assertEqual(
            {
                'operation_type': 'net_iface_ip_op',
    ...
    def test_load_from_etcd(self):
        ...
        nii = NetIfaceIPOp.from_db(op_uuid)
        self.assertNotEqual(None, nii)
        self.assertEqual('queued', nii.state.value)

    def test_object_mapping(self):
        self.assertTrue(
            NetIfaceIPOp.object_type in OPERATION_NAMES_TO_CLASSES)
        self.assertTrue(
            NetIfaceIPOp.object_type in OBJECT_NAMES_TO_CLASSES)
