...
from shakenfist.etcd_schema.operations.node_net_op import create_and_enqueue
from shakenfist.etcd_schema.operations.node_net_op import current_version
from shakenfist.etcd_schema.operations.node_net_op import model
from shakenfist.etcd_schema.operations.node_net_op import model_tasks
...
class NodeNetOpTestCase(base.ShakenFistTestCase):
    ...
    def test_create_and_enqueue(self, _mock_time, _mock_id):
        ...
        self.assertEqual(
            ...
            self.mock_etcd.get_raw(f'/sf/node_net_op/{op_uuid}')
        )
        self.assertEqual(
            {
                'value': 'queued',
            ...
            self.mock_etcd.get_raw(
                f'/sf/attribute/node_net_op/{op_uuid}/state')
        )
        self.assertEqual(
            {
    ...
    def test_object_mapping(self):
        self.assertTrue(
            NodeNetOp.object_type in OPERATION_NAMES_TO_CLASSES)
