...
from shakenfist.etcd_schema.operations.net_ip_op import create_and_enqueue
from shakenfist.etcd_schema.operations.net_ip_op import current_version
from shakenfist.etcd_schema.operations.net_ip_op import model
from shakenfist.etcd_schema.operations.net_ip_op import model_tasks
...
class NetIPOpTestCase(base.ShakenFistTestCase):
    ...
    def test_model(self):
        ...
        d = model(
            ...
            network_uuid=u2,
            ip='8.8.8.8',
            tasks=[model_tasks.route_address],
        ...
        self.assertEqual(['route_address'], serialized['tasks'])
    ...
    def test_create_and_enqueue(self, _mock_time, _mock_id):
        ...
        self.assertEqual(CLUSTER_OPERATIONS.net_ip_op, op_type)
        ...
        self.assertEqual(
            ...
            self.mock_etcd.get_raw(f'/sf/net_ip_op/{op_uuid}')
        )
        self.assertEqual(
            {
                'value': 'queued',
            ...
            self.mock_etcd.get_raw(
                f'/sf/attribute/net_ip_op/{op_uuid}/state')
        )
        self.assertEqual(
            {
    ...
    def test_load_from_etcd(self):
        ...
        nii = NetIPOp.from_db(op_uuid)
        self.assertNotEqual(None, nii)
        self.assertEqual('queued', nii.state.value)

    def test_object_mapping(self):
        self.assertTrue(NetIPOp.object_type in OPERATION_NAMES_TO_CLASSES)
        self.assertTrue(NetIPOp.object_type in OBJECT_NAMES_TO_CLASSES)
