from shakenfist.exceptions import (
    NoURLImageFetchTaskException,
...
class QueueTask(object):
    ...
    def json_dump(self):
        return {"task": self._name, "version": self._version}
...
class InstanceTask(QueueTask):
    ...
    def json_dump(self):
        return {
            **super(InstanceTask, self).json_dump(),
...
class ErrorInstanceTask(InstanceTask):
    ...
    def json_dump(self):
        return {
            **super(ErrorInstanceTask, self).json_dump(),
...
class NetworkTask(QueueTask):
    ...
    def json_dump(self):
        return {
            **super(NetworkTask, self).json_dump(),
...
class ImageTask(QueueTask):
    ...
    def json_dump(self):
        return {**super(ImageTask, self).json_dump(), "url": self._url}
...
class FetchImageTask(ImageTask):
    ...
    def json_dump(self):
        return {
            **super(FetchImageTask, self).json_dump(),
            "instance_uuid": self._instance_uuid,
        }
