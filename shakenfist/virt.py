...
def _get_disk_device_base(bus):
    bases = {"ide": "hd", "scsi": "sd", "usb": "sd", "virtio": "vd"}
...
class Instance(object):
    def __init__(self, db_entry):
        ...
        self.snapshot_path = os.path.join(config.get("STORAGE_PATH"), "snapshots")
            ...
            "devices": [
                {
                    ...
                    "present_as": _get_defaulted_disk_type(
                        self.db_entry["disk_spec"][0]
                    ),
    ...
    def create(self, lock=None):
                    ...
                    if disk.get("present_as", "cdrom") == "cdrom":
                        ...
                        if config.get("DISK_FORMAT") == "qcow":
                            ...
                            with util.RecordedOperation("resize image", self):
                                resized_image_path = img.resize([lock], disk["size"])
        ...
        db.persist_block_devices(self.db_entry["uuid"], self.db_entry["block_devices"])
    ...
    def delete(self):
        ...
        with util.RecordedOperation("release network addresses", self):
            for ni in db.get_instance_interfaces(self.db_entry["uuid"]):
                ...
                with db.get_lock(
                    "ipmanager", None, ni["network_uuid"], ttl=120, op="Instance delete"
                ):
    ...
    def _make_config_drive(self, disk_path):
        ...
        iso = pycdlib.PyCdlib()
        iso.new(
            interchange_level=4, joliet=True, rock_ridge="1.09", vol_ident="config-2"
        )

        # We're only going to pretend to do the most recent OpenStack version
        iso.add_directory("/openstack", rr_name="openstack", joliet_path="/openstack")
        iso.add_directory(
        ...
        iso.add_directory(
            "/openstack/latest", rr_name="latest", joliet_path="/openstack/latest"
        )
        ...
        md = json.dumps(
            {
                ...
                "public_keys": {"mykey": self.db_entry["ssh_key"]},
            }
        ).encode("ascii")
        iso.add_fp(
            io.BytesIO(md),
    ...
    def power_on(self):
        if not os.path.exists(self.xml_file):
            db.enqueue_instance_error(
                self.db_entry["uuid"], "missing domain file in power on"
            )
        ...
        if not instance:
            ...
            if not instance:
                db.enqueue_instance_error(
                    self.db_entry["uuid"], "power on failed to create domain"
                )
    ...
    def snapshot(self, all=False):
        ...
        for d in disks:
            ...
            with util.RecordedOperation("snapshot %s" % d["device"], self):
                self._snapshot_device(d["path"], os.path.join(snappath, d["device"]))
                db.create_snapshot(
                    snapshot_uuid, d["device"], self.db_entry["uuid"], time.time()
                )
    ...
    def pause(self):
        ...
        db.add_event("instance", self.db_entry["uuid"], "pause", "complete", None, None)
