...
def resolve(name):
    resp = requests.get(UBUNTU_URL, headers={"User-Agent": util.get_user_agent()})
    ...
    num_to_name = {}
    name_to_num = {}
    dir_re = re.compile(r'.*<a href="(.*)/">.*Ubuntu Server ([0-9]+\.[0-9]+).*')
    ...
    url = config.get("DOWNLOAD_URL_UBUNTU") % {"vernum": vernum, "vername": vername}
    log = LOG.withField("url", url)

    # Retrieve check sum file
    checksum_url = UBUNTU_URL + "/" + vername + "/current/MD5SUMS"
    resp = requests.get(checksum_url, headers={"User-Agent": util.get_user_agent()})
    if resp.status_code != 200:
        raise exceptions.HTTPError(
            "Failed to fetch %s, status code %d" % (checksum_url, resp.status_code)
        )
