...
class BaseTestCase(testtools.TestCase):
    ...
    def _make_namespace(self, name, key):
        ...
        return apiclient.Client(
            base_url=self.system_client.base_url, namespace=name, key=key
        )
    ...
    def _log_instance_events(self, instance_uuid):
        ...
        self._log_events(
            instance_uuid, self.system_client.get_instance_events(instance_uuid)
        )
    ...
    def _log_netns(self):
        ...
        sys.stderr.write("----------------------- netns -----------------------\n")
        out, err = processutils.execute(
            "sudo ip netns", shell=True, check_exit_code=[0, 1]
        )
        for line in out:
            sys.stderr.write(line)
        sys.stderr.write("----------------------- end netns -----------------------\n")
    ...
    def _await_instance_event(self, instance_uuid, operation, message=None, after=None):
    ...
    def _await_image_download_error(self, url, after=None):
        return self._await_image_event(url, "fetch", "Name or service not known", after)

    def _await_image_event(self, url, operation, message=None, after=None):
        start_time = time.time()
        while time.time() - start_time < 300:
    ...
    def _test_ping(self, instance_uuid, network_uuid, ip, expected, attempts=1):
        ...
        if expected != actual:
            ...
            self.fail(
                "Ping test failed. Expected %s != actual %s.\nout: %s\nerr: %s\n"
...
class BaseNamespacedTestCase(BaseTestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.namespace = "ci-%s-%s" % (namespace_prefix, self._uniquifier())
    ...
    def setUp(self):
        ...
        self.test_client = self._make_namespace(self.namespace, self.namespace_key)
