...
def main():
    ...
    disks = [
        {"base": "cirros", "size": 4, "type": "disk"},
        {"base": "ubuntu", "size": 8, "type": "disk"},
    ]

    # Some network specs we randomly select from
    networks = []
    for i in range(5):
        n = c.allocate_network("192.168.%d.0/24" % (50 + i), True, True, randomName())
    ...
    while True:
        ...
        if choice == 1:
            n = c.allocate_network(
                "192.168.%d.0/24" % (50 + random.randint(0, 100)),
                True,
                True,
                randomName(),
            )
